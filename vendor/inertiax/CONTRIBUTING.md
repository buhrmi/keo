...
Thank you for your interest in contributing to Inertia X! Your contributions help make this project better for everyone.

Inertia X is maintained as a monorepo using [Bun](https://bun.com). Below you'll find an overview of the repository and how to get your development environment running.
...
```sh
cd packages/core && bun dev
cd packages/react && bun dev
```
...
These commands automatically set a `PACKAGE` environment variable that tells the Node.js test server which adapter to serve. For example, when running `bun test:react`, the test server loads the React test application.
...
```sh
bun test:react -g "partial reload"
```
...
```sh
bun dev:test-app:react
bun dev:test-app:svelte
bun dev:test-app:vue
```
...
```sh
bun test:react -g "your feature"
bun test:svelte -g "your feature"
bun test:vue -g "your feature"
```
...
Each playground has its own bun script:

```sh
bun playground:react
bun playground:svelte
bun playground:vue
```
